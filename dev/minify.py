...
Per-file – used by download_to_device.py for incremental builds:
    python dev/minify.py --source vendor/HexCurrent/hexcurrent.py --artifact EEPROM/hexcurrent.mpy
"""
import argparse
import ast
import string
...
_PRESERVE_HEXCURRENT: frozenset[str] = frozenset({
    # BadgeOS app lifecycle
    "update", "draw", "background_task", "run", "run_sync",
    # Public state
    "config", "VERSION", "settings",
    # Public properties / methods
    "logging", "capture_seconds", "show_message",
    "update_settings", "set_logging",
    # SensorBase / INA226 public interface
    "begin", "reset", "close", "open", "read_sample_if_ready",
    "read_interval", "i2c_addr",
    # Dunder / short names not worth renaming
...
_PRESERVE_HEXDRIVE2: frozenset[str] = frozenset({
    ...
    "background_update",
    "__init__",
    # Public API called by BadgeBot / HexManager
...
_PRESERVE_GPS: frozenset[str] = frozenset({
    # BadgeOS app lifecycle
    "update", "draw", "background_update",
    "__init__", "__app_export__",
...
_PRESERVE_CAFFEINE: frozenset[str] = frozenset({
    # BadgeOS app lifecycle
    "update", "draw", "background_update",
    "__init__", "__app_export__",
    # Public state
    ...
    "deinit",
    # DRV2605 / Pause property names that have @name.setter decorators.
    # If renamed the @setter decorator would reference the old (now-gone) name → NameError.
    "mode", "library", "realtime_value", "duration",
})
...
MINIFIABLE: tuple[MinifySpec, ...] = (
    MinifySpec(
        ROOT / "vendor" / "HexCurrent" / "hexcurrent.py",
        ROOT / "EEPROM" / "hexcurrent.mpy",
        _PRESERVE_HEXCURRENT,
    ),
    MinifySpec(
    ...
    MinifySpec(
        ROOT / "EEPROM" / "caffeine.py",
        ROOT / "EEPROM" / "caffeine.mpy",
        _PRESERVE_CAFFEINE,
    ),
)
